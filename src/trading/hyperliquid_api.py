...
class HyperliquidAPI:
    ...
    async def set_leverage(self, asset, leverage, is_cross=True):
        ...
        Args:
            asset: Market symbol to configure.
            leverage: Requested leverage (will be rounded to an integer >= 1).
            is_cross: Cross margin flag passed to Hyperliquid.

        Returns:
            Exchange response payload or an error dict when update fails.
        """
        try:
            lev = int(round(float(leverage)))
            if lev < 1:
                lev = 1
            return await self._retry(lambda: self.exchange.update_leverage(lev, asset, is_cross))
